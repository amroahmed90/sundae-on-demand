...
describe("test images response from server for scoops and toppings", () => {
  ...
  test("display images for each topping option from server", async () => {
    ...
    // confirm alt text of images
    const altText = toppingImages.map((element) => (element as HTMLImageElement).alt);
...
describe("test scoops and toppings Options component", () => {
  test("initial conditions for scoops", async () => {
    ...
    expect(scoopNames.map((element) => element.textContent)).toEqual(["Mint Chip", "Salted Caramel"]);

    // confirm select options
    const selectOptions = await screen.findAllByTestId(/select/i);
    expect(selectOptions.map((element) => element.textContent)).toEqual(["012", "012"]); // 0, 1, 2 => to be edited when functionality is added
  });
  ...
  test("initial conditions for toppings", async () => {
    ...
    const scoopHeading = await screen.findByRole("heading", { name: /toppings/i });
    expect(scoopHeading).toBeInTheDocument();
    ...
    expect(scoopNames.map((element) => element.textContent)).toEqual(["M&Ms", "Hot fudge"]);
